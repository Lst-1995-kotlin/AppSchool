...
class PostModifyFragment : Fragment() {
    ...
    fun check():Boolean{
        if(!mainActivity.inputCheck(fragmentPostModifyBinding.textInputEditTextPostModifySubject, "제목")) return false
        if(!mainActivity.inputCheck(fragmentPostModifyBinding.textInputEditTextPostModifyText, "내용")) return false
        return true
    }
