    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activityMainBinding.run {
            button.setOnClickListener {
                ...
                // 작은 아이콘
                bulider.setSmallIcon(android.R.drawable.ic_menu_search)
                // 큰 아이콘
                val bitmap = BitmapFactory.decodeResource(resources, R.mipmap.ic_launcher)
                bulider.setLargeIcon(bitmap)
                // 숫자 설정
                bulider.setNumber(100)
                // 타이틀 설정
                bulider.setContentTitle("Content Title 1")
                // 메시지 설정
                bulider.setContentText("Content Text 1")
                ...
                // 같은 값으로 메시지를 계속 보여주면 메시지가 갱신 것이고
